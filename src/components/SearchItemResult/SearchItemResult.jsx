...
export default function SearchItemResult({ products, filters }) {
  ...
  const handleSortBy = (sortBy, order) => {
  ...
  const handleActiveClassName = (value) =>
  ...
  return (
    <div>
      <S.SortBar>
        ...
        <S.SortOptions>
          <S.SortOption
            className={handleActiveClassName("view")}
            onClick={() => handleSortBy("view")}
          >
            Phổ biến
          </S.SortOption>
          <S.SortOption
            className={handleActiveClassName("createdAt")}
            onClick={() => handleSortBy("createdAt")}
          >
            Mới nhất
          </S.SortOption>
          <S.SortOption
            className={handleActiveClassName("sold")}
          ...
          <S.SortByPrice
            onChange={(e) => handleSortBy(...e.target.value.split(":"))}
            className={handleActiveClassName("price")}
        ...
        <S.MiniPageController>
          ...
          <S.ButtonPrev onClick={btnPrevClick} disabled={pagination.page === 1}>
            <svg
              viewBox="0 0 7 11"
          ...
          <S.ButtonNext onClick={btnNextClick} disabled={pagination.page === pagination.page_size}>
            <svg
              viewBox="0 0 7 11"
