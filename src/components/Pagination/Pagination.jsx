    ...
    <div>
      <S.Pagination>
        {items.map(({page, type, selected}, index) => {
          let children = null;
          if(type === "starts-ellipsis" || type === "end-ellipsis"){
            children = <S.ButtonNoOutLine disabled key={index}>...</S.ButtonNoOutLine>
            ...
            children = <S.ButtonNoOutLine key={index} onClick={() => goToPage(page)} className={classNames({active: selected})}>{page}</S.ButtonNoOutLine>
          }
          return children;
        })}
        {/* <S.ButtonIcon>
